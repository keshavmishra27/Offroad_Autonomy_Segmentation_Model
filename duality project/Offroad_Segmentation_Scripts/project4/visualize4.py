"""
Visualize Script — Project 4 (ENet)
Colorizes raw segmentation mask images (single-channel with raw pixel values)
for visual inspection. Same logic as projects 1-3.
...
output_folder = os.path.join(input_folder, "colorized_enet")
...
for image_file in sorted(image_files):
    ...
    for v in u:
        if v not in color_map:
            # Deterministic colour based on value so output is consistent
