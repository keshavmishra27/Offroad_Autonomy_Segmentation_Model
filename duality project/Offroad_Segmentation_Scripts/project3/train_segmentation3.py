"""
Segmentation Training Script — Project 3 (U-Net + MobileNetV2)
Trains a U-Net with ImageNet-pretrained MobileNetV2 encoder for offroad semantic segmentation.

CPU-optimised strategy:
  - U-Net decoder + MobileNetV2 backbone via segmentation_models_pytorch
  - Standard CrossEntropy loss (no Hungarian matching → much faster)
  - Image size 256×256
...
# MobileNetV2 ImageNet weights load via Hugging Face Hub; Windows often cannot use
# symlinked cache entries. Downloads still work — this only suppresses the warning.
...
class HRNetSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Load and resize mask (NEAREST to preserve class IDs)
...
def main():
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Step 1: Class distribution + class weights for loss
    # ──────────────────────────────────────────────────────────────────────
    ...
    # ──────────────────────────────────────────────────────────────────────
    # Load best model and compute per-class IoU
    # ──────────────────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    ...
    # Final checkpoint already saved during training (unet_mobilenetv2_best.pth)
